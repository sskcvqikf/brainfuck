...
void
buffer::increment_dataptr(int n)
{ return increment_dataptr_impl(n); }

void
buffer::decrement_dataptr(int n)
{ return decrement_dataptr_impl(n); }

void
buffer::increment_byte(int n)
{ return increment_byte_impl(n); }

void
buffer::decrement_byte(int n)
...
void
vector_buffer::increment_byte_impl(int n)
{ vec_.at(idx_) += n; }

void
vector_buffer::decrement_byte_impl(int n)
{ vec_.at(idx_) -= n; }

void
vector_buffer::increment_dataptr_impl(int n)
{
    auto tmp = idx_ + n;
    if (tmp > 30000)
...
void
vector_buffer::decrement_dataptr_impl(int n)
{
    auto tmp = idx_ - n;
    if (tmp < 0)
